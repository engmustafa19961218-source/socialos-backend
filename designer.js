...
function addProdImgs(input) {
  ...
  if (!file) return;
  dsProdFile = file;
  dsProdUrl = URL.createObjectURL(file);
  ...
  if (grid) {
    grid.innerHTML = '';
    const div = document.createElement('div');
    div.style.cssText = 'position:relative;border-radius:10px;overflow:hidden;aspect-ratio:1;border:2px solid var(--accent)';
    const img = document.createElement('img');
    img.src = dsProdUrl;
    img.style.cssText = 'width:100%;height:100%;object-fit:cover;display:block';
    ...
    btn.onclick = () => { dsProdFile = null; dsProdUrl = null; grid.innerHTML = ''; if(thumbRow) thumbRow.innerHTML = '<span style="font-size:.75rem;color:#888">ارفع صور المنتج أولاً</span>'; };
