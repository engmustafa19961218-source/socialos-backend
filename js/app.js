...
function goBack() {
  ...
  if (pg) pg.classList.add('active');
  const ldr = {home:ldHome,orders:ldOrds,products:ldProds,customers:ldCusts,knowledge:ldKb,decisions:ldDec,board:ldBoard,approvals:ldApprov,analytics:ldAna,'analytics-adv':ldAdvAna,'heatmap':ldHeatmap,'digital-team':loadTeamPage,'team-meeting':ldTeamMeeting,coupons:ldCoups,loyalty:ldLoy,'referral':ldReferral,'bulk-wa':ldWa,marketplace:ldMkt,social:ldSocial,ads:ldAds,posts:ldPosts,comments:ldCommentsPosts,invoices:ldInvoices,identity:ldIdentity,designer:ldDesigner,'video':loadVideoPage,team:ldTeam,notifications:ldNotifs,security:ldSec,backup:ldBkps,emergency:ldEm,inventory:ldInv,settings:ldSets,profile:ldProf,employee:ldEmp,training:ldCorrs,delivery:ldDelivery,retargeting:ldRetargeting,'employee-log':ldEmpLog,'session-detail':()=>{},departments:loadDepartmentsPage,ai:loadAIPage};
...
// دوال مساعدة
function loadAna() { ldAna(); }
function showPage(pageId) { sp(pageId.replace('page-',''), null); }
