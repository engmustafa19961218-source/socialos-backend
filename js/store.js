async function ldCoups(){
  ...
  list.innerHTML=d.coupons.map(c=>`<div style="background:#111111;border:1px solid rgba(201,168,76,.1);border-radius:14px;padding:14px;margin-bottom:10px"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px"><span style="font-family:monospace;font-weight:700;font-size:.9rem;color:#E8C96A">${esc(c.code)}</span><span style="background:${c.is_active?'rgba(34,197,94,.15)':'rgba(239,68,68,.15)'};color:${c.is_active?'#22c55e':'#ef4444'};padding:2px 10px;border-radius:20px;font-size:.68rem;font-weight:700">${c.is_active?'✅ فعال':'❌ معطل'}</span></div><div style="font-size:.75rem;color:#9A9590;margin-bottom:10px">${c.type==='percent'?'نسبة':'مبلغ'}: <b style="color:#E8C96A">${c.value}${c.type==='percent'?'%':''}</b> | استخدم: ${c.used_count}/${c.max_uses}</div><button style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);border-radius:10px;padding:7px 14px;color:#ef4444;font-size:.75rem;cursor:pointer;font-family:'Tajawal',sans-serif" onclick="delCoup(${c.id})">🗑️ حذف</button></div>`).join('');
}
...
async function ldLoy(){
  ...
  list.innerHTML=d.customers.map(c=>`<div style="background:#111111;border:1px solid rgba(201,168,76,.1);border-radius:14px;padding:14px;margin-bottom:10px"><div style="display:flex;align-items:center;justify-content:space-between"><div><div style="font-weight:700;font-size:.88rem">${esc(c.customer_name||'-')}</div><div style="font-size:.72rem;color:#9A9590;margin-top:2px" dir="ltr">${esc(c.customer_phone)}</div></div><div style="display:flex;gap:14px;text-align:center"><div><div style="font-size:1.2rem;font-weight:900;color:#E8C96A">${c.points}</div><div style="font-size:.6rem;color:#5A5550">نقطة</div></div><div><div style="font-size:.88rem;font-weight:700;color:#22c55e">${c.total_earned}</div><div style="font-size:.6rem;color:#5A5550">مكتسب</div></div><div><div style="font-size:.88rem;font-weight:700;color:#9A9590">${c.total_redeemed}</div><div style="font-size:.6rem;color:#5A5550">مستخدم</div></div></div></div></div>`).join('');
}
...
async function ldTeam(){
  ...
  list.innerHTML=d.members.map(m=>`<div style="background:#111111;border:1px solid rgba(201,168,76,.1);border-radius:14px;padding:14px;margin-bottom:10px"><div style="display:flex;align-items:center;gap:12px"><div style="width:44px;height:44px;border-radius:13px;background:linear-gradient(135deg,#8B6914,#C9A84C);display:flex;align-items:center;justify-content:center;font-size:1.1rem;color:#080808;font-weight:900;flex-shrink:0">${esc(m.name?.[0]||'؟')}</div><div style="flex:1"><div style="font-weight:700;font-size:.88rem">${esc(m.name)}</div><div style="font-size:.72rem;color:#9A9590;margin-top:2px" dir="ltr">${esc(m.email)}</div><span style="font-size:.65rem;background:rgba(201,168,76,.1);color:#C9A84C;padding:2px 8px;border-radius:10px;margin-top:4px;display:inline-block">${rl[m.role]||m.role}</span></div><button style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);border-radius:10px;padding:8px 12px;color:#ef4444;font-size:.75rem;cursor:pointer;font-family:'Tajawal',sans-serif" onclick="delTeam(${m.id})">🗑️</button></div></div>`).join('');
}
...
async function ldSec(){
  ...
  ll.innerHTML=ld.map(l=>`<div style="background:#111111;border:1px solid rgba(201,168,76,.1);border-radius:14px;padding:10px 14px;margin-bottom:8px"><div style="display:flex;align-items:center;justify-content:space-between"><div><div style="font-size:.82rem;font-weight:600">${esc(l.action)}</div><div style="font-size:.68rem;color:#9A9590;margin-top:1px">${esc(l.entity)}</div></div><div style="font-size:.65rem;color:#5A5550;text-align:left">${new Date(l.created_at).toLocaleString('ar')}</div></div></div>`).join('');
}
